interface i3c_if (
  ...
  task automatic sample_one_byte(input string msg, output bit [7:0] data);
    data = '0;
    for (int i = 7; i >= 0; i--) begin
      sample_target_data(data[i]);
      `uvm_info(msg_id, $sformatf("sampled %s bit[%0d]=%0b", msg, i, data[i]), UVM_DEBUG)
    end
    `uvm_info(msg_id, $sformatf("sampled %s=0x%02h", msg, data), UVM_HIGH)
  endtask : sample_one_byte

  task automatic sample_i3c_data_byte_and_t_bit(input string msg, output bit [7:0] data,
                                                 output bit t_bit);
    t_bit = 1'b0;

    sample_one_byte(msg, data);
    sample_target_data(t_bit);
    `uvm_info(msg_id, $sformatf("sampled %s=0x%02h T-bit=%0b", msg, data, t_bit), UVM_HIGH)
  endtask : sample_i3c_data_byte_and_t_bit
