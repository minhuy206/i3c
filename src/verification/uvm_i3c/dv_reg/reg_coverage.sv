class reg_coverage extends uvm_subscriber #(reg_seq_item);
  ...
  bit          cmd_dw0_valid;
  bit [31:0]   cmd_dw0;
  // Mirrors RTL gate: sw_reset only asserts when i3c_fsm_idle_i is high.
  // Updated on every HC_STATUS read; initialized to 1 (idle after hard reset).
  bit          last_fsm_idle;
  ...
  covergroup cg_dat_entry;
    ...
    cp_access_type: coverpoint reg_item.is_write {
      bins read = {0};
      bins write = {1};
    }

    cp_device: coverpoint dat_entry.device {
      bins i3c = {0};
      bins legacy_i2c = {1};
    }
  ...
  covergroup cg_cmd_desc;
    ...
    cp_cmd_rnw: coverpoint cmd_rnw iff (cmd_has_rnw) {
      bins write = {0};
      bins read = {1};
    }

    cp_cmd_toc: coverpoint cmd_toc {
      bins continue_transfer = {0};
      bins terminate_on_completion = {1};
    }

    cp_cmd_wroc: coverpoint cmd_wroc {
      bins no_response = {0};
      bins response_required = {1};
    }
    ...
    cx_regular_rnw_len: cross cp_cmd_rnw, cp_cmd_data_len
      iff (cmd_attr == RegularTransfer);
  ...
  covergroup cg_resp_desc;
    ...
    cp_resp_tid: coverpoint resp_desc.tid {
      bins tid[] = {[0:15]};
    }
    ...
    cp_resp_reserved_zero: coverpoint resp_desc.__rsvd23_16 {
      bins zero = {8'h00};
    }
  endgroup
  ...
  covergroup cg_hc_control;
    ...
    cp_bus_enable: coverpoint csr_bus_enable {
      bins disabled = {0};
      bins enabled = {1};
    }
    ...
    cp_broadcast_enable: coverpoint csr_broadcast_enable {
      bins disabled = {0};
      bins enabled = {1};
    }

    cp_hc_abort: coverpoint csr_hc_abort {
      bins cleared = {0};
      bins set = {1};
    }

    cp_hc_abort_transition: coverpoint csr_hc_abort {
      bins set_transition = (0 => 1);
      bins clear_transition = (1 => 0);
    }
  ...
  covergroup cg_timing_csr;
    ...
    cp_timing_default: coverpoint timing_is_default {
      bins non_default_value = {0};
      bins default_value = {1};
    }

    cp_timing_reserved_upper: coverpoint timing_reserved_upper {
      bins clear = {0};
      bins write_attempt = {1};
    }
  ...
  virtual function void write(reg_seq_item t);
    ...
    // REG-015 covers accepted software requests at the queue ports. Exact FIFO
    // push/pop handshakes and empty-pop behavior remain owned by RTL/SVA.
    ...
    if (!t.is_write && (t.addr == ADDR_HC_STATUS))
      last_fsm_idle = t.rdata[HC_STS_FSM_IDLE_BIT];

    // Mirror RTL: sw_reset only asserts (and flushes staging) when FSM is idle.
    // A soft-reset write while busy is a no-op in hardware; don't drop DWORD0.
  ...
  virtual function void report_phase(uvm_phase phase);
    ...
    `uvm_info("REG_COVERAGE", $sformatf(
              {"cg_reg_access=%0.2f%% cg_dat_entry=%0.2f%% ",
               "cg_cmd_desc=%0.2f%% cg_resp_desc=%0.2f%% ",
