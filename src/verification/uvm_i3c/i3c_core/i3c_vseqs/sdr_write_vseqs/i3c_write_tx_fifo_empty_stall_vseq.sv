class i3c_write_tx_fifo_empty_stall_vseq extends i3c_base_vseq;
  ...
  localparam logic [3:0]  FLOW_STALL_WRITE = 4'd10;
  ...
  virtual task body();
    ...
    `DV_CHECK_EQ(dev_seq.sampled_data.size(), DATA_LENGTH,
                 "SDRW_006: sampled byte count mismatch")
    ...
    `uvm_info(`gfn, "SDRW_006 I3C write TX FIFO empty stall/recovery checks passed",
              UVM_LOW)
  endtask
  ...
  virtual task wait_for_flow_state(logic [3:0] exp_state, string state_name,
    ...
    `uvm_fatal(`gfn, $sformatf("SDRW_006: timed out waiting for flow_active.%s",
                               state_name))
  endtask
