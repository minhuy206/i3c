class i3c_write_back_to_back_vseq extends i3c_base_vseq;
  ...
  virtual task run_back_to_back_case(bit broadcast_header_enable);
    ...
    cfg0 = make_transfer_cfg(
        .ctxt($sformatf(
            "SDRW_006 %s back_to_back[0]", private_addr_mode_name(broadcast_header_enable)
        )),
        .seq_name($sformatf(
    ...
    cfg1 = make_transfer_cfg(
        .ctxt($sformatf(
            "SDRW_006 %s back_to_back[1]", private_addr_mode_name(broadcast_header_enable)
        )),
        .seq_name($sformatf(
    ...
    cfg2 = make_transfer_cfg(
        .ctxt($sformatf(
            "SDRW_006 %s back_to_back[2]", private_addr_mode_name(broadcast_header_enable)
        )),
        .seq_name($sformatf(
    ...
    check_all_queues_empty("after SDRW_006 back_to_back");
    disable_dut();

    `uvm_info(`gfn, $sformatf(
                  "SDRW_006 result: mode=%s queued_cmds=3 sampled_bytes={%0d,%0d,%0d} observed_rstart={%0b,%0b,%0b}",
