class i3c_write_multi_dat_idx_vseq extends i3c_base_vseq;
  ...
  task body();
    ...
    `uvm_info(`gfn,
              "SDRW_009 conclusion: SDR private writes select the programmed DAT index and preserve that selection across toc=0 continuation",
              UVM_LOW)
  endtask
  ...
  virtual task run_multi_dat_idx_case(bit broadcast_header_enable);
    ...
    cfg0 = make_transfer_cfg(
        ...
        .seq_name($sformatf(
            "sdrw009_%s_dev_seq0", private_addr_mode_name(broadcast_header_enable)
        )),
        .tid(4'h9),
        .dev_idx(5'd0),
        .target_addr(7'h08),
    ...
    cfg1 = make_transfer_cfg(
        ...
        .seq_name($sformatf(
            "sdrw009_%s_dev_seq1", private_addr_mode_name(broadcast_header_enable)
        )),
        .tid(4'hB),
        .dev_idx(5'd1),
        .target_addr(7'h12),
    ...
    check_all_queues_empty(
        $sformatf("after SDRW_009 %s multi DAT", private_addr_mode_name(broadcast_header_enable)));

    `uvm_info(`gfn, $sformatf(
                  "SDRW_009 result: mode=%s case=multi_dat target_addrs={0x%02h,0x%02h} sampled_addrs={0x%02h,0x%02h} sampled_bytes={%0d,%0d}",
  ...
  virtual task run_toc_zero_multi_dat_idx_case(bit broadcast_header_enable);
    ...
    cfg0 = make_transfer_cfg(
        .ctxt($sformatf(
            "SDRW_009 %s toc0 DAT[0]", private_addr_mode_name(broadcast_header_enable)
        )),
        .seq_name($sformatf(
            "sdrw009_%s_toc0_dev_seq0", private_addr_mode_name(broadcast_header_enable)
        )),
        .tid(4'hC),
        .dev_idx(5'd0),
        .target_addr(7'h08),
    ...
    cfg1 = make_transfer_cfg(
        .ctxt($sformatf(
            "SDRW_009 %s toc0 DAT[1]", private_addr_mode_name(broadcast_header_enable)
        )),
        .seq_name($sformatf(
            "sdrw009_%s_toc0_dev_seq1", private_addr_mode_name(broadcast_header_enable)
        )),
        .tid(4'hD),
        .dev_idx(5'd1),
        .target_addr(7'h12),
    ...
    check_all_queues_empty(
        $sformatf(
        "after SDRW_009 %s toc0 multi-DAT", private_addr_mode_name(broadcast_header_enable)));

    `uvm_info(`gfn, $sformatf(
                  "SDRW_009 result: mode=%s case=toc0_multi_dat target_addrs={0x%02h,0x%02h} sampled_addrs={0x%02h,0x%02h} rstart_count=%0d first_observed_rstart=%0b second_observed_rstart=%0b",
                  private_addr_mode_name(broadcast_header_enable), cfg0.target_addr,
