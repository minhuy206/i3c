class bus_rx_byte_and_bit_order_vseq extends bus_base_vseq;
  ...
  virtual task run_pattern_case(int unsigned pattern_idx, bit [6:0] dynamic_addr);
    ...
    cfg = make_transfer_cfg(
        .ctxt($sformatf("BUS_011 pattern %0d", pattern_idx)),
        .seq_name($sformatf("bus011_dev_seq_%0d", pattern_idx)),
        .tid(4'(pattern_idx + 1)),
        .dev_idx(5'd0),
    ...
    `DV_CHECK_EQ(rx_words.size(), exp_words.size(),
                 $sformatf("BUS_011 pattern %0d word count mismatch", pattern_idx))
    foreach (exp_words[i]) begin
      `DV_CHECK_EQ(rx_words[i], exp_words[i], $sformatf(
                   "BUS_011 pattern %0d word %0d byte order mismatch", pattern_idx, i))
    end
  endtask
