class i3c_entdaa_rx_fifo_partial_overflow_vseq extends i3c_base_vseq;
  ...
  virtual task run_rx_fifo_overflow_case(int unsigned free_words);
    ...
    ctxt            = $sformatf("ERR_017 ENTDAA RX FIFO overflow free_words=%0d", free_words);
    ...
    prefill_rx_fifo_count(prefill_words, $sformatf(
                          "after ERR_017 RX prefill free_words=%0d", free_words));
    ...
    dev_seq = i3c_device_response_seq::type_id::create(
        $sformatf("err017_entdaa_dev_seq_free%0d", free_words));
    ...
    check_error_resp_fields(resp, RESP_OVL, daa_cmd.tid, committed_words * 4, ctxt);

    check_queue_flags(rx_paths.name, rx_paths.full_bit, rx_paths.empty_bit, 1'b1, 1'b0, $sformatf(
                      "before ERR_017 RX drain free_words=%0d", free_words));
    for (int unsigned i = 0; i < prefill_words; i++) begin
      read_rx_data(rx_word);
      `DV_CHECK_EQ(rx_word, make_prefill_word(i), $sformatf(
                   "ERR_017 free_words=%0d: drained RX prefill word[%0d] mismatch", free_words, i))
    end
    for (int unsigned i = 0; i < committed_words; i++) begin
      read_rx_data(rx_word);
      `DV_CHECK_EQ(rx_word, make_daa_result_word(i, pid, bcr, dcr, assigned_addr), $sformatf(
                   "ERR_017 free_words=%0d: committed DAA result word[%0d] mismatch", free_words, i
                   ))
    end

    check_all_queues_empty($sformatf(
                           "after ERR_017 ENTDAA RX FIFO overflow free_words=%0d", free_words));
    `uvm_info(
        `gfn,
        $sformatf(
            "ERR_017 result: free_words=%0d resp=0x%08h assigned=0x%02h prefill_words=%0d committed_words=%0d",
  ...
  virtual function bit [31:0] make_daa_result_word(
      int unsigned index, bit [47:0] pid, bit [7:0] bcr, bit [7:0] dcr, bit [6:0] assigned_addr);
    case (index)
      0: make_daa_result_word = pid[47:16];
      1: make_daa_result_word = {pid[15:0], bcr, dcr};
      2: make_daa_result_word = {25'h0, assigned_addr};
      default: make_daa_result_word = '0;
    endcase
  endfunction
