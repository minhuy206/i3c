class i3c_entdaa_rx_fifo_partial_overflow_vseq extends i3c_base_vseq;
  ...
  virtual task run_rx_fifo_overflow_case(int unsigned free_words);
    ...
    ctxt            = $sformatf("ERR_006 ENTDAA RX FIFO overflow free_words=%0d", free_words);
    ...
    prefill_rx_fifo_count(prefill_words, $sformatf(
                          "after ERR_006 RX prefill free_words=%0d", free_words));
    ...
    dev_seq = i3c_device_response_seq::type_id::create(
        $sformatf("err006_entdaa_dev_seq_free%0d", free_words));
    ...
    check_queue_flags(rx_paths.name, rx_paths.full_bit, rx_paths.empty_bit, 1'b1, 1'b0, $sformatf(
                      "before ERR_006 RX drain free_words=%0d", free_words));
    for (int unsigned i = 0; i < prefill_words; i++) begin
      read_rx_data(rx_word);
      `DV_CHECK_EQ(rx_word, make_prefill_word(i), $sformatf(
                   "ERR_006 free_words=%0d: drained RX prefill word[%0d] mismatch", free_words, i))
    end
    ...
    check_all_queues_empty($sformatf(
                           "after ERR_006 ENTDAA RX FIFO overflow free_words=%0d", free_words));
    `uvm_info(
        `gfn,
        $sformatf(
            "ERR_006 result: free_words=%0d resp=0x%08h assigned=0x%02h prefill_words=%0d committed_words=%0d",
