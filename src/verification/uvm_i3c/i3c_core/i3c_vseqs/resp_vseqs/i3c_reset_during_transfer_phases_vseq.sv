class i3c_reset_during_transfer_phases_vseq extends bus_base_vseq;
  ...
  localparam string ENTDAA_STATE_PATH =
  ...
  virtual function immediate_data_trans_desc_t make_immediate_cmd(bit [3:0] tid,
                                                                   bit [2:0] dtt);
    ...
    cmd.wroc              = 1'b1;
    cmd.dev_idx           = 5'd0;
    cmd.def_or_data_byte1 = 8'hA1;
    cmd.data_byte2        = 8'hB2;
    cmd.data_byte3        = 8'hC3;
    cmd.data_byte4        = 8'hD4;
    return cmd;
  endfunction
  ...
  virtual task run_data_rx_reset_case();
    ...
    configure_target_and_enable();
    for (int unsigned i = 0; i < 8; i++) read_data.push_back(8'h40 + i[7:0]);
    cfg = make_transfer_cfg(
        .ctxt(ctxt),
  ...
  virtual task wait_for_address_ack_phase(string ctxt);
    for (int unsigned cycle = 0; cycle < PhaseTimeoutCycles; cycle++) begin
      @(posedge p_sequencer.cfg.m_i3c_agent_cfg.vif.clk_i);
      if ((hdl_read_uint_lsb(FLOW_STATE_PATH, 4) == FSM_INIT_I3C_WRITE) &&
          (hdl_read_uint_lsb(ISSUE_PHASE_PATH, 8) == PHASE_ADDR_ACK) &&
          (hdl_read_uint_lsb(RX_FLOW_STATE_PATH, 3) == RX_READ_BIT)) return;
    end
  ...
  virtual task wait_for_data_tx_phase(string ctxt);
    for (int unsigned cycle = 0; cycle < PhaseTimeoutCycles; cycle++) begin
      @(posedge p_sequencer.cfg.m_i3c_agent_cfg.vif.clk_i);
      if ((hdl_read_uint_lsb(FLOW_STATE_PATH, 4) == FSM_ISSUE_CMD) &&
  ...
  virtual task wait_for_data_rx_phase(string ctxt);
    for (int unsigned cycle = 0; cycle < PhaseTimeoutCycles; cycle++) begin
      @(posedge p_sequencer.cfg.m_i3c_agent_cfg.vif.clk_i);
      if ((hdl_read_uint_lsb(FLOW_STATE_PATH, 4) == FSM_ISSUE_CMD) &&
          (hdl_read_uint_lsb(RX_FLOW_STATE_PATH, 3) == RX_READ_BYTE)) return;
    end
  ...
  virtual task wait_for_write_resp_stall(string ctxt);
    for (int unsigned cycle = 0; cycle < PhaseTimeoutCycles; cycle++) begin
      @(posedge p_sequencer.cfg.m_i3c_agent_cfg.vif.clk_i);
      if ((hdl_read_uint_lsb(FLOW_STATE_PATH, 4) == FSM_WRITE_RESP) &&
          hdl_read_bit(resp_paths.write_valid_path) &&
          !hdl_read_bit(resp_paths.write_ready_path)) return;
    end
  ...
  virtual task check_bus_released(string ctxt);
    `DV_CHECK_EQ(hdl_read_bit(SCL_O_PATH), 1'b1,
                 $sformatf("%s: SCL output not released", ctxt))
    `DV_CHECK_EQ(hdl_read_bit(SDA_O_PATH), 1'b1,
                 $sformatf("%s: SDA output not at idle value", ctxt))
    `DV_CHECK_EQ(hdl_read_bit(SDA_OE_PATH), 1'b0,
                 $sformatf("%s: SDA output enable not released", ctxt))
    ...
    `DV_CHECK_EQ(hdl_read_bit(SCL_BUS_PATH), 1'b1,
                 $sformatf("%s: SCL bus not high", ctxt))
    `DV_CHECK_EQ(hdl_read_bit(SDA_BUS_PATH), 1'b1,
                 $sformatf("%s: SDA bus not high", ctxt))
  endtask
