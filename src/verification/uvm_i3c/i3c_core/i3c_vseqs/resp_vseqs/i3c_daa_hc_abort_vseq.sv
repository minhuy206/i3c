class i3c_daa_hc_abort_vseq extends i3c_base_vseq;
  ...
  virtual task run_mid_identity_abort();
    ...
    `DV_CHECK_RANDOMIZE_FATAL(stimulus, "ERR_009 mid-identity stimulus randomization failed")

    write_dat_entry(0, 7'h50, stimulus.assigned_addr, 1'b0);
    daa_cmd = make_daa_cmd(4'd8, 5'd0, 4'd1);

    dev_seq = i3c_device_response_seq::type_id::create("err009_mid_id_dev_seq");
  ...
  virtual task run_recovery_after_abort();
    ...
    `DV_CHECK_RANDOMIZE_FATAL(stimulus, "ERR_009 recovery stimulus randomization failed")

    write_dat_entry(0, 7'h50, stimulus.assigned_addr, 1'b0);
    daa_cmd = make_daa_cmd(4'd9, 5'd0, 4'd1);

    dev_seq = i3c_device_response_seq::type_id::create("err009_recovery_dev_seq");
    configure_joining_device(dev_seq, stimulus);
  ...
  virtual task run_address_phase_abort();
    ...
    `DV_CHECK_RANDOMIZE_FATAL(stimulus, "ERR_009 address-phase stimulus randomization failed")

    write_dat_entry(1, 7'h50, stimulus.assigned_addr, 1'b0);
    daa_cmd = make_daa_cmd(4'd10, 5'd1, 4'd1);

    dev_seq = i3c_device_response_seq::type_id::create("err009_addr_phase_dev_seq");
    configure_joining_device(dev_seq, stimulus);
