class i3c_write_tx_fifo_underflow_vseq extends i3c_base_vseq;
  ...
  virtual task body();
    ...
    foreach (broadcast_modes[mode_idx]) begin
      foreach (toc_modes[toc_idx]) begin
        ...
        run_tx_fifo_empty_underflow_ovl_case(1'b1, broadcast_modes[mode_idx],
                                             toc_modes[toc_idx], 4);
        run_tx_fifo_empty_underflow_ovl_case(1'b1, broadcast_modes[mode_idx],
                                             toc_modes[toc_idx], 8);
      end
  ...
  virtual task run_tx_fifo_underflow_ovl_case(bit is_i3c, bit broadcast_header_enable, bit toc);
    ...
    cfg = make_transfer_cfg(
        ...
        .seq_name($sformatf(
            "err007_%s_%s_toc%0d_dev_seq", is_i3c ? "i3c" : "i2c",
            private_addr_mode_name(broadcast_header_enable), toc
        )),
        .tid(4'd6),
        .dev_idx(5'd0),
    ...
    `uvm_info(`gfn, $sformatf(
                  "ERR_007 result: device=%s mode=%s toc=%0d requested_len=%0d supplied_words=1 sampled_bytes=%0d",
                  is_i3c ? "I3C" : "I2C",
  ...
  virtual task run_tx_fifo_empty_underflow_ovl_case(bit is_i3c, bit broadcast_header_enable,
    ...
    `uvm_info(`gfn, $sformatf(
                  "ERR_007 result: device=%s mode=%s toc=%0d requested_len=%0d supplied_words=0 sampled_bytes=%0d",
                  is_i3c ? "I3C" : "I2C",
                  private_addr_mode_name(broadcast_header_enable), toc, data_length,
                  dev_seq.sampled_data.size()), UVM_LOW)
  endtask
  ...
  virtual task run_late_refill_after_underflow_case(bit is_i3c, bit broadcast_header_enable);
    ...
    cfg = make_transfer_cfg(
        ...
        .seq_name($sformatf(
            "err007_%s_%s_late_refill_dev_seq", is_i3c ? "i3c" : "i2c",
    ...
    `uvm_info(`gfn, $sformatf(
                  "ERR_007 result: device=%s mode=%s requested_len=%0d sampled_bytes_before_late_refill=%0d late_refill_words=1 sw_reset_flushed_queues=1 recovery=pass",
                  is_i3c ? "I3C" : "I2C",
  ...
  virtual task run_underflow_with_queued_followup_case(bit broadcast_header_enable);
    ...
    cfg0 = make_transfer_cfg(
        .ctxt($sformatf("ERR_007 %s underflow with queued followup",
                        private_addr_mode_name(broadcast_header_enable))),
        .seq_name($sformatf("err007_%s_underflow_queued_dev_seq0",
                            private_addr_mode_name(broadcast_header_enable))),
        .tid(4'hA), .dev_idx(5'd0), .target_addr(dynamic_addr), .is_i3c(1'b1),
        .ack_address(1'b1), .ack_data(1'b1), .tx_before_cmd(1'b1), .wait_device_done(1'b1),
        .start_with_broadcast_header(broadcast_header_enable), .data_length(DATA_LENGTH),
        .settle_before_cmd(0), .timeout_cycles(0));
    cfg1 = make_transfer_cfg(
        .ctxt($sformatf("ERR_007 %s clean command after queued underflow",
                        private_addr_mode_name(broadcast_header_enable))),
        .seq_name($sformatf("err007_%s_underflow_queued_dev_seq1",
                            private_addr_mode_name(broadcast_header_enable))),
        .tid(4'hB), .dev_idx(5'd0), .target_addr(dynamic_addr), .is_i3c(1'b1),
        .ack_address(1'b1), .ack_data(1'b1), .tx_before_cmd(1'b1), .wait_device_done(1'b1),
        .start_with_broadcast_header(broadcast_header_enable), .data_length(0),
        .settle_before_cmd(0), .timeout_cycles(0));

    dev_cfg0 = cfg0;
    dev_cfg0.data_length = ACTUAL_LENGTH;
    start_ordered_device_responses(dev_cfg0, 1'b0, no_read_data, dev_seq0, cfg1, 1'b0,
                                   no_read_data, dev_seq1);
    build_random_tx_words(ACTUAL_LENGTH, exp_data, tx_words);
    write_tx_words(tx_words);
  ...
  virtual task run_recovery_write_case(bit is_i3c, bit broadcast_header_enable,
    ...
    cfg = make_transfer_cfg(
        ...
        .tid(4'hC), .dev_idx(5'd0),
        .target_addr(is_i3c ? dynamic_addr : static_addr), .is_i3c(is_i3c),
        .ack_address(1'b1), .ack_data(1'b1), .tx_before_cmd(1'b1), .wait_device_done(1'b1),
        .start_with_broadcast_header(is_i3c && broadcast_header_enable), .data_length(4),
        .settle_before_cmd(0), .timeout_cycles(0));
