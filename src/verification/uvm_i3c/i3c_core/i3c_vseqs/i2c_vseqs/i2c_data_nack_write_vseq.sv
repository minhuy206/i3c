class i2c_data_nack_write_vseq extends i3c_base_vseq;
  ...
  task body();
    ...
    `uvm_info(
        `gfn,
        "I2C_005 conclusion: legacy I2C write stops on a data-byte NACK and accepts a following legal write",
        UVM_LOW)
  endtask
  ...
  virtual task run_data_nack_case();
    ...
    cfg = make_transfer_cfg(
        .ctxt($sformatf("I2C_005 data NACK byte %0d", NACK_BYTE_IDX)),
    ...
    check_i2c_address_phase(dev_seq, cfg);
    check_nacked_write_data(dev_seq, exp_data, cfg.ctxt);
    check_error_resp_fields(resp, RESP_I2C_DATA_NACK, cfg.tid, nacked_transfer_length(), cfg.ctxt);

    `uvm_info(`gfn,
              $sformatf(
                  "I2C_005 data NACK result: resp=0x%08h sampled_addr=0x%02h sampled_bytes=%0d",
  ...
  virtual task run_recovery_write_case();
    ...
    check_i2c_address_phase(dev_seq, cfg);
    check_sampled_write_data(dev_seq, exp_data, RECOVERY_LENGTH, cfg.ctxt);
    check_all_queues_empty("after I2C_005 recovery write");

    `uvm_info(`gfn, $sformatf("I2C_005 recovery result: resp=0x%08h sampled_bytes=%0d", resp,
  ...
  virtual function int unsigned nacked_transfer_length();
    return NACK_BYTE_IDX + 1;
  endfunction
  ...
  virtual task check_i2c_address_phase(i3c_device_response_seq dev_seq,
    ...
    `DV_CHECK_EQ(dev_seq.sampled_dir, 1'b0, $sformatf("%s: sampled direction should be write",
                                                      cfg.ctxt))
  endtask

  virtual task check_nacked_write_data(i3c_device_response_seq dev_seq, byte_queue_t exp_data,
                                       string ctxt);
    `DV_CHECK_EQ(dev_seq.sampled_data.size(), nacked_transfer_length(),
                 $sformatf("%s: sampled byte count should stop at the NACKed byte", ctxt))

    for (int unsigned i = 0; i < nacked_transfer_length(); i++) begin
      if ((i < dev_seq.sampled_data.size()) && (i < exp_data.size())) begin
        `DV_CHECK_EQ(dev_seq.sampled_data[i], exp_data[i],
                     $sformatf("%s: sampled byte[%0d] mismatch before NACK", ctxt, i))
      end
    end
  endtask
