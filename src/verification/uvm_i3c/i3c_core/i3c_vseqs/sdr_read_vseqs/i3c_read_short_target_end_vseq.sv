class i3c_read_short_target_end_vseq extends i3c_base_vseq;
  `uvm_object_utils(i3c_read_short_target_end_vseq)

  localparam int unsigned NUM_CASES = 4;

  function new(string name = "i3c_read_short_target_end_vseq");
    super.new(name);
  endfunction

  task body();
    int unsigned req_lengths[NUM_CASES] = '{4, 8, 8, 9};
    int unsigned actual_lengths[NUM_CASES] = '{1, 3, 4, 5};

    configure_dut();
    write_dat_entry(0, 7'h50, 7'h08, 1'b0);

    foreach (req_lengths[case_idx]) begin
      run_short_case(case_idx, req_lengths[case_idx], actual_lengths[case_idx]);
    end

    `uvm_info(`gfn, "SDRR_003 I3C short read target-end checks passed", UVM_LOW)
  endtask

  virtual task run_short_case(int unsigned case_idx, int unsigned requested_length,
                              int unsigned actual_length);
    transfer_stimulus_cfg_t cfg;
    byte_queue_t            read_data;
    word_queue_t            rx_words;
    bit [31:0]              resp;
    i3c_device_response_seq dev_seq;

    `DV_CHECK_LT(actual_length, requested_length,
                 $sformatf("SDRR_003 case %0d must end before requested length", case_idx))

    build_payload(case_idx, actual_length, read_data);

    cfg                  = make_transfer_cfg(
        $sformatf("SDRR_003 req %0d actual %0d", requested_length, actual_length),
        $sformatf("sdrr003_dev_seq_%0d_%0d", requested_length, actual_length),
        4'(case_idx + 1),
        5'd0,
        7'h08,
        1'b1,
        requested_length
    );
    cfg.wait_device_done = 1'b1;

    expect_scoreboard_resp_error(4'h7, cfg.tid, 16'(actual_length), cfg.ctxt);
    run_read_stimulus_words_with_actual_len(cfg, read_data, actual_length, rx_words, resp,
                                            dev_seq);

    `DV_CHECK_EQ(dev_seq.done, 1'b1,
                 $sformatf("SDRR_003 req %0d actual %0d: device response did not finish",
                           requested_length, actual_length))
    `DV_CHECK_EQ(dev_seq.sampled_addr, 7'h08,
                 $sformatf("SDRR_003 req %0d actual %0d: target address mismatch",
                           requested_length, actual_length))
    `DV_CHECK_EQ(dev_seq.sampled_dir, 1'b1,
                 $sformatf("SDRR_003 req %0d actual %0d: transfer direction should be read",
                           requested_length, actual_length))

    `DV_CHECK_EQ(resp[31:28], 4'h7,
                 $sformatf("SDRR_003 req %0d actual %0d: expected I3cShortReadErr response",
                           requested_length, actual_length))
    `DV_CHECK_EQ(resp[27:24], cfg.tid,
                 $sformatf("SDRR_003 req %0d actual %0d: response TID mismatch",
                           requested_length, actual_length))
    `DV_CHECK_EQ(resp[15:0], 16'(actual_length),
                 $sformatf("SDRR_003 req %0d actual %0d: response length mismatch",
                           requested_length, actual_length))

    check_all_queues_empty($sformatf("after SDRR_003 req %0d actual %0d",
                                     requested_length, actual_length));
  endtask

  virtual function void build_payload(int unsigned case_idx, int unsigned actual_length,
                                      ref byte_queue_t read_data);
    read_data.delete();

    for (int unsigned i = 0; i < actual_length; i++) begin
      read_data.push_back(8'(8'h90 + (case_idx * 8) + i));
    end
  endfunction

endclass
