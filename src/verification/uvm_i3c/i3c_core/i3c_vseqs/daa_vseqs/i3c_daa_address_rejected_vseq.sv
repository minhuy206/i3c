class i3c_daa_address_rejected_vseq extends i3c_base_vseq;
  ...
  virtual task body();
    ...
    for (int unsigned i = 0; i < 2; i++) begin
      reject_seq[i] =
          i3c_device_response_seq::type_id::create($sformatf("daa006_reject_seq_%0d", i));
    ...
    `uvm_info(`gfn, $sformatf("DAA_006 result: resp=0x%08h rejected_addr=0x%02h pid=0x%012h", resp,
                              stimulus.assigned_addr, stimulus.pid), UVM_LOW)
