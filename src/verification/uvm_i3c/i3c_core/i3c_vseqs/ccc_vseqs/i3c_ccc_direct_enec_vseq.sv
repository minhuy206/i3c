class i3c_ccc_direct_enec_vseq extends i3c_base_vseq;
  ...
  virtual task body();
    ...
    bcast_seq                 = i3c_device_response_seq::type_id::create("ccc004_bcast_seq");
    ...
    target_seq               = i3c_device_response_seq::type_id::create("ccc004_target_seq");
    ...
    fork : device_response
      ...
      begin
        wait_for_device_request_issued(bcast_seq, "CCC_004 broadcast leg");
    ...
    poll_idle();
    wait_for_device_done(bcast_seq, "CCC_004 direct ENEC broadcast leg", 2000);
    wait_for_device_done(target_seq, "CCC_004 direct ENEC target leg", 2000);
    settle_cycles(4);

    `DV_CHECK_EQ(bcast_seq.sampled_addr, 7'h7e, "ccc_004: broadcast address mismatch")
    `DV_CHECK_EQ(bcast_seq.sampled_dir, 1'b0, "ccc_004: broadcast direction should be write")
    `DV_CHECK_EQ(bcast_seq.sampled_data.size(), 1, "ccc_004: expected direct ENEC opcode")
    if (bcast_seq.sampled_data.size() >= 1) begin
      `DV_CHECK_EQ(bcast_seq.sampled_data[0], 8'h80, "ccc_004: direct ENEC opcode mismatch")
    end
    `DV_CHECK_EQ(bcast_seq.sampled_t_bit.size(), 1, "ccc_004: expected opcode T-bit")
    if (bcast_seq.sampled_t_bit.size() >= 1) begin
      `DV_CHECK_EQ(bcast_seq.sampled_t_bit[0], ~^8'h80,
    ...
    `DV_CHECK_EQ(target_seq.sampled_addr, target_addr, "ccc_004: target address mismatch")
    `DV_CHECK_EQ(target_seq.sampled_dir, 1'b0, "ccc_004: target direction should be write")
    `DV_CHECK_EQ(target_seq.sampled_data.size(), 1, "ccc_004: expected one Target Events byte")
    if (target_seq.sampled_data.size() >= 1) begin
      `DV_CHECK_EQ(target_seq.sampled_data[0], event_byte,
                   "ccc_004: Target Events byte mismatch")
    end
    `DV_CHECK_EQ(target_seq.sampled_t_bit.size(), 1, "ccc_004: expected event byte T-bit")
