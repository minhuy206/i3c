module tb_pad_model_sva (
  ...
  assert property (@(posedge clk_i) disable iff (!rst_ni) !$isunknown(
      {
                       dut_sda_oe_i,
                       dut_sda_o_i,
                       dut_sel_od_pp_i,
  ...
  assert property (@(posedge clk_i) disable iff (!rst_ni) !unsafe_contention)
    ...
    $error(
        "tb_pad_model_sva: BUS_013 unsafe SDA contention dut_oe=%0b dut_sda=%0b dut_pp=%0b dev_sda=%0b dev_pp=%0b sda_bus=%0b hc_abort=%0b",
        dut_sda_oe_i,
        dut_sda_o_i,
        dut_sel_od_pp_i,
        device_sda_o_i,
