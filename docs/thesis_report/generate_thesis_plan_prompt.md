...
Paste this entire prompt into a Claude Code session (or Claude chat with file access) to generate the plan.
...
The project implements a simplified MIPI I3C Basic v1.1.1 Active Controller in SystemVerilog, derived from the CHIPS Alliance `i3c-core` reference (≈92% LoC reduction). The thesis covers RTL design, UVM 1.2 functional verification, and optional FPGA validation.
...
### Thesis structure & master plan
- `docs/thesis_report/thesis_master_plan.md` — the existing plan (may be stale; treat as a baseline to verify, not ground truth)
...
For every chapter (1–10 + Appendices), produce a sub-section with:
...
- Which chapters depend on FPGA validation (write last or skip to Path B)
...
1. **`flow_active.sv`** — the flagship module. The spec (`09_flow_active_spec.md`) claims 13 states. Verify the actual enum. Check if HC abort / interrupt states have been added in recent commits (branch is `feat/hc-abort-intr-sdrw-flow`).
...
5. **Regression targets** — check `src/verification/Makefile` for targets. The master plan mentions `make regression`, `make sdrw_regression`, `make sdrr_regression`, etc. Verify each target exists and maps to the described set of tests.
...
7. **SVA checkers** — the master plan mentions `sva/` directory under `i3c_core/`. Check whether `flow_active_sva.sv`, `csr_registers_sva.sv`, etc. actually exist and are bound in `tb_i3c_top.sv`.
...
The reference used **only 4 chapters** and merged design+verification into Chapter 3. This is simpler than the 10-chapter structure in `thesis_master_plan.md`. **Flag this difference explicitly** and ask the writer whether they want to follow the reference's compact 4-chapter model or expand to the full 10-chapter model. This is a key structural decision.
