...
| ID | Test Name | Objective | Preconditions | Stimulus / Actions | Expected Results | Related Modules / Interfaces | Priority | Coverage Goals |
|---|---|---|---|---|---|---|---|---|
| SDRW_001 | `i3c_regular_write_4b_existing` | Verify baseline 4-byte SDR private write. | DAT[0] is I3C dynamic address `0x08`; device ACKs. | Run `i3c_write_vseq` with TX word `DEAD_BEEF`. | Bus write uses dynamic address+W; data byte order follows the project TX FIFO packing contract; RESP success length 4. | Full DUT, UVM scoreboard | High | `cp_cmd_attr`, `cp_dir`, `cp_len_4` |
...
| I2C_005 | `i2c_data_nack_write` | Verify I2C data-byte NACK. | Target ACKs address and NACKs byte M. | Issue multi-byte write. | Transfer stops after the NACKed byte per I2C/project error policy; RESP error is `Nack`; next legal transfer passes. | `flow_active` | High | `cp_data_ack`, `cp_resp_err` |
...
| I3C private read | SDRR_001..SDRR_009 | `cp_dir`, `cp_rx_partial_dword`, `cp_read_tbit` |
