...
|---|---|---|---|---|---|---|---|---|
| CSR_001 | `csr_reset_defaults` | Verify reset-visible register defaults. | Reset applied and released. | Read `HC_CONTROL`, `HC_STATUS`, all timing registers, `QUEUE_STATUS`, and DAT[0..15]. | Enable is 0; FSM idle/status reflects idle; queue flags show empty; DAT entries are 0; timing defaults match the CSR/module specification. | `csr_registers`, reg bus | High | `cp_reset`, `cp_csr_addr` |
...
| CSR_003 | `csr_broadcast_addr_enable_control` | Verify private-transfer broadcast header control bit behavior. | Controller idle and CMD FIFO empty. | Read `HC_CONTROL[BROADCAST_ADDR_ENABLE]` after reset; set and clear the bit through `HC_CONTROL`; set only this bit without `HC_CONTROL[0]`. | Reset value is 0; writable set/clear behavior is correct; setting `BROADCAST_ADDR_ENABLE` alone does not enable the controller and does not start a bus transaction. | `csr_registers`, `flow_active` | High | `cp_broadcast_addr_enable`, `cp_enable` |
...
| SDRW_002 | `i3c_write_broadcast_header_enabled` | Verify SDR private write with broadcast-header preamble enabled. | DAT[0] is I3C dynamic address `0x08`; `HC_CONTROL[BROADCAST_ADDR_ENABLE]=1`; device ACKs broadcast header and target address. | Issue a 4-byte regular write with known TX data. | Bus frame is `START + 0x7e/W + ACK + Sr + 0x08/W + ACK + data/T-bit + STOP`; monitor/scoreboard treat the preamble and target phase as one complete transaction; RESP success length matches data bytes. | `flow_active`, `scl_generator`, UVM scoreboard | High | `cp_broadcast_addr_enable`, `cp_private_start_prefix`, `cp_first_address` |
| SDRW_003 | `i3c_regular_write_len_sweep` | Verify write lengths and TX packing. | Device ACKs address and write data. | Write lengths 1,2,3,4,5,7,8,16 using patterned TX words. | Exact byte count transmitted; correct byte order; RESP length equals bytes transferred. | `flow_active`, `hci_queues`, `bus_tx_flow` | High | `cp_data_len`, `cp_tx_word_boundary` |
| SDRW_004 | `i3c_regular_write_data_patterns` | Verify data integrity over patterns. | Device ACKs all phases. | Send all-zero, all-one, walking-one, alternating, and random data. | Device monitor/scoreboard observes exact expected bytes and generated T-bits. | `bus_tx_flow`, `i3c_monitor`, scoreboard | Medium | `cp_data_pattern` |
| SDRW_005 | `i3c_write_tbit_parity_generation` | Verify SDR write T-bit generation. | I3C device target; bus monitor can sample T-bit. | Send data bytes with even and odd parity. | T-bit makes the 8-bit data byte plus T-bit odd parity, equivalent to `~^data_byte`, for every write byte. | `flow_active`, `bus_tx_flow` | High | `cp_tbit_write_parity` |
| SDRW_006 | `i3c_write_addr_nack` | Verify address NACK response. | Device configured to NACK dynamic address. | Issue regular write. | No data bytes are transmitted; STOP/recovery occurs; RESP error is `AddrHeader`. | `flow_active`, `bus_rx_flow`, RESP FIFO | High | `cp_resp_err`, `cp_addr_ack` |
| SDRW_007 | `i3c_write_tx_fifo_empty_stall` | Verify TX underflow stall/recovery. | Write length exceeds initially available TX data. | Issue write, delay additional TX data, then provide it. | Controller stalls without corrupting bus data and resumes when TX data is available. | `flow_active`, `hci_queues`, `scl_generator` | High | `cp_stall_type`, `cp_tx_empty` |
| SDRW_008 | `i3c_write_toc_zero` | Verify SDR regular write continuation when `toc=0`. | I3C device ACKs; a second SDR regular I3C command is already queued. | Queue first write with `toc=0` and second write with `toc=1`. | First write completes data/T-bit then emits `Sr` instead of STOP; second command starts after `Sr`; final STOP occurs only after the `toc=1` command; both RESPs report success with matching TID/length. | `flow_active`, `scl_generator` | Medium | `cp_toc`, `cp_rstart_continuation` |
| SDRW_009 | `i3c_write_toc_zero_broadcast_header_once` | Verify broadcast-header preamble is not repeated across a private write continuation. | `HC_CONTROL[BROADCAST_ADDR_ENABLE]=1`; I3C device ACKs; a second SDR regular I3C write command is already queued. | Queue first write with `toc=0` and second write with `toc=1`. | First command emits `START + 0x7e/W + ACK + Sr + dynamic address`; continuation after the repeated START does not emit a second `0x7e` preamble; a new preamble is allowed only after a STOP and later private transfer. | `flow_active`, `scl_generator`, UVM scoreboard | Medium | `cp_broadcast_addr_enable`, `cp_toc`, `cp_private_start_prefix` |
| SDRW_010 | `i3c_write_back_to_back` | Verify write-to-write sequencing. | Multiple TX data words and commands queued. | Queue several write commands with unique TIDs and data. | Commands execute FIFO order; each RESP TID/length matches; no stale TX data. | `hci_queues`, `flow_active`, scoreboard | High | `cp_sequence_type`, `cross_tid_x_order` |

### 4.5 I3C SDR Private Read

| ID | Test Name | Objective | Preconditions | Stimulus / Actions | Expected Results | Related Modules / Interfaces | Priority | Coverage Goals |
|---|---|---|---|---|---|---|---|---|
| SDRR_001 | `i3c_regular_read_4b_existing` | Verify baseline 4-byte SDR private read without broadcast-header preamble. | DAT[0] is I3C dynamic address `0x08`; `HC_CONTROL[BROADCAST_ADDR_ENABLE]=0`; target returns four bytes. | Run `i3c_read_vseq`. | Bus read starts with `START + 0x08/R + ACK`; no `0x7e` preamble is emitted; RX FIFO word matches the target bytes packed by the project RX FIFO contract, e.g. `32'hBEBA_FECA` for the existing stimulus; RESP success length 4. | Full DUT, UVM scoreboard | High | `cp_cmd_attr`, `cp_dir`, `cp_len_4`, `cp_private_start_prefix` |
| SDRR_002 | `i3c_read_broadcast_header_enabled` | Verify SDR private read with broadcast-header preamble enabled. | DAT[0] is I3C dynamic address `0x08`; `HC_CONTROL[BROADCAST_ADDR_ENABLE]=1`; target ACKs broadcast header and returns read data. | Issue a 4-byte regular read. | Bus frame is `START + 0x7e/W + ACK + Sr + 0x08/R + ACK + read data/T-bit + STOP`; monitor/scoreboard treat the preamble and target phase as one complete transaction; RX data and RESP length match expected bytes. | `flow_active`, `scl_generator`, `bus_rx_flow`, UVM scoreboard | High | `cp_broadcast_addr_enable`, `cp_private_start_prefix`, `cp_first_address` |
| SDRR_003 | `i3c_regular_read_len_sweep` | Verify read lengths and RX packing. | Device returns enough bytes. | Read lengths 1,2,3,4,5,7,8,16. | RX FIFO contains all bytes in little-endian DWORD packing; partial final DWORD is preserved. | `flow_active`, `hci_queues` | High | `cp_data_len`, `cp_rx_partial_dword` |
| SDRR_004 | `i3c_read_short_target_end` | Verify target early end handling. | Device returns T-bit end before requested length. | Request N bytes; target ends after M<N bytes. | RESP error is `I3cShortReadErr`; RX FIFO contains only transferred bytes. | `flow_active`, `bus_rx_flow`, RESP FIFO | High | `cp_short_read`, `cp_resp_err` |
| SDRR_005 | `i3c_read_target_more_than_requested` | Verify controller termination at requested length. | Device can provide more bytes. | Request N bytes while target indicates continuation beyond N. | Controller terminates after N bytes with STOP when `toc=1`; no extra RX bytes are stored. | `flow_active`, `scl_generator` | High | `cp_read_end_policy` |
| SDRR_006 | `i3c_read_addr_nack` | Verify address NACK response on read. | Device NACKs dynamic address+R. | Issue regular read. | No RX data is written; RESP error is `AddrHeader`; controller recovers. | `flow_active`, `bus_rx_flow` | High | `cp_resp_err`, `cp_addr_ack` |
| SDRR_007 | `i3c_read_rx_fifo_full_stall` | Verify RX backpressure. | RX FIFO is full before final data flush. | Issue read, hold RX full, then drain RX. | Controller stalls safely and resumes; no data loss. | `flow_active`, `hci_queues`, `scl_generator` | High | `cp_rx_full`, `cp_stall_type` |
| SDRR_008 | `i3c_read_data_patterns` | Verify read data integrity. | Device sequence can drive arbitrary data. | Return zero, one, alternating, walking, and random byte patterns. | RX FIFO contents match target data exactly. | `i3c_driver`, `bus_rx_flow`, scoreboard | Medium | `cp_data_pattern` |
| SDRR_009 | `i3c_read_no_parity_error_on_end_tbit` | Verify read T-bit semantics. | Target ends transfer with T-bit=0. | Run read where final T-bit is 0. | Per I3C SDR read semantics, T-bit=0 is an end-of-data indicator, not a parity error; response is Success if the requested length is met, otherwise `I3cShortReadErr`. | `flow_active` | High | `cp_read_tbit`, `cp_resp_err` |
| SDRR_010 | `i3c_read_toc_zero` | Verify SDR regular read continuation when `toc=0`. | I3C device returns requested read data; a second SDR regular I3C command is already queued. | Queue first read with `toc=0` and second write with `toc=1`. | First read completes requested data/T-bit then emits `Sr` instead of STOP; second command starts after `Sr`; final STOP occurs only after the `toc=1` command; RX data and both RESPs match expected length/TID. | `flow_active`, `scl_generator`, `bus_rx_flow` | Medium | `cp_toc`, `cp_rstart_continuation`, `cp_dir_read_to_write` |
| SDRR_011 | `i3c_read_toc_zero_broadcast_header_once` | Verify broadcast-header preamble is not repeated across a private read continuation. | `HC_CONTROL[BROADCAST_ADDR_ENABLE]=1`; I3C device returns requested read data; a second SDR regular I3C command is already queued. | Queue first read with `toc=0` and second command with `toc=1`. | First command emits `START + 0x7e/W + ACK + Sr + dynamic address`; continuation after the repeated START does not emit a second `0x7e` preamble before the final STOP; RX data and RESPs remain correct. | `flow_active`, `scl_generator`, `bus_rx_flow`, UVM scoreboard | Medium | `cp_broadcast_addr_enable`, `cp_toc`, `cp_private_start_prefix` |

### 4.6 Immediate Data Transfer

| ID | Test Name | Objective | Preconditions | Stimulus / Actions | Expected Results | Related Modules / Interfaces | Priority | Coverage Goals |
|---|---|---|---|---|---|---|---|---|
| IMM_001 | `i3c_immediate_write_smoke_existing` | Verify baseline immediate write descriptor flow. | DAT[0] I3C device; target ACKs. | Run `i3c_smoke_vseq`. | Two inline data bytes are transmitted according to descriptor `dtt` and project packing rules; RESP success. | `flow_active`, `bus_tx_flow`, UVM | High | `cp_imm`, `cp_len_2` |
...
| ERR_002 | `resp_addr_header_error` | Verify address NACK error. | Device NACKs address. | Run I3C write/read, I2C write/read, and direct CCC address phases. | RESP error is `AddrHeader` for every supported command class with an address/header NACK; no data phase occurs after address NACK. | `flow_active` | High | `cp_resp_err`, `cp_addr_ack` |
...
| ERR_005 | `resp_unreachable_error_codes` | Document defined but unused error codes. | Code review plus directed unsupported stimuli. | Attempt CRC, Frame, Ovl, HcAborted, NotSupported, Parity scenarios. | Error codes outside the specified controller scope are N/A or enhancement requests, not positive coverage; any in-scope error code must have a spec-derived stimulus and expected response. | `i3c_pkg`, `flow_active` | Low | `cp_error_code_reachability` |
...
| SCL timing and stalls | BUS_005, BUS_006, SDRW_007, SDRR_007, I2C_008 | `cp_timing_mode`, `cp_stall_type` |
| I3C private write | SDRW_001..SDRW_010 | `cp_dir`, `cp_data_len`, `cp_tbit_write_parity`, `cp_private_start_prefix` |
| I3C private read | SDRR_001..SDRR_011 | `cp_dir`, `cp_rx_partial_dword`, `cp_read_tbit`, `cp_private_start_prefix` |
...
| Broadcast-header private preamble | CSR_003, SDRW_001/002/009, SDRR_001/002/011, I2C_003 | `cp_broadcast_addr_enable`, `cp_private_start_prefix`, `cp_first_address` |
