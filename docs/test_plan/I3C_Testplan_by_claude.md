...
| 7.3 | ENTDAA | i3c_entdaa_no_device | Verify NoDev path when no device responds to ENTDAA | AddressAssignment dev_count=1; no device pulls SDA low for ACK after SendDAAHeader | entdaa_fsm reaches NoDev; entdaa_controller sets no_device; RESP issued normally | High | entdaa_fsm.sv | cp_entdaa_dev_count.0 |
...
| 7.8 | ENTDAA | i3c_entdaa_pid_shift_64bit | Verify 64-bit PID/BCR/DCR shift register captures correct value | Device drives known 64-bit ID = 0x12345678_9ABCDEF0; observe id_shift_q after ReceiveID | PID[47:0]=0x123456789ABC, BCR=0xDE, DCR=0xF0 captured in entdaa_fsm id_shift_q | High | entdaa_fsm.sv | cp_entdaa_dev_count.1 |
...
| `cp_reset_point` | FSM state at time of reset assertion | `Idle`, `WaitForCmd`, `FetchDAT`, `Imm_or_Regular` (I3CWriteImm/I2CWriteImm), `Stall` (StallWrite/StallRead), `IssueCmd_addr`, `IssueCmd_data`, `IssueCmd_T_or_parity`, `DAA_mid` (entdaa_fsm ReceiveID), `WriteResp` |
