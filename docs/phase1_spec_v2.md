...
| 0x5  | `Nack`                       | DAA address rejected twice (ENTDAA retry exhausted) | ✓   |
...
> (Crc, Parity, Frame) are declared in the enum but not generated by this SDR-only design — HDR
> modes are out of scope. `I2cDataNackOrI3cBusAborted` (0x9) **is** generated — it is the code
> `flow_active`'s `map_resp_err_status()` returns for a data-phase NACK / bus abort; `Nack` (0x5) is
> reserved for the DAA double-reject path, not data NACK. See `flow_active.sv` lines 288–306.
