...
| DAA-006 | Assigned address | `cg_daa_assigned_address.cx_address_class_parity`: low `0x08..0x1f`, middle `0x20..0x5f`, high `0x60..0x77` × parity 0/1; chỉ sample round có đủ byte assigned-address. Reserved assigned-address values are prevented before the byte is transmitted and are covered by DAA-010. | DAA single/multi/boundary/rejected vseqs | Scoreboard check + `Implemented` address × parity |
