...
> Estimated LoC: ~385 lines (combined)
...
| --------- | --------- | --------------------------------- |
| `bus_tx`  | `xbus_tx` | Bit-level timing engine (5 states)|
...
| `bus_error_o`   | Output    | 1     | Bus error (tied to `'0`)                  |
...
Same as `bus_tx_flow` (`t_r_i`, `t_su_dat_i`, `t_hd_dat_i`).
...
**Error detection:** If both `req_byte_i` and `req_bit_i` are asserted simultaneously, `req_error_o` asserts and the FSM returns to Idle.

### 5.2. bus_tx FSM (5 states)

```systemverilog
typedef enum logic [2:0] {
  Idle             = 3'd0,
  AwaitClockNegedge= 3'd1,
  SetupData        = 3'd2,
  TransmitData     = 3'd3,
  HoldData         = 3'd4
} tx_state_e;
...
**Timing counter (`tcount`) — helper enum:**

```systemverilog
typedef enum logic [1:0] {
  tSetupData = 2'd0,  // Load: t_r_i + t_su_dat_i
  tHoldData  = 2'd1,  // Load: t_hd_dat_i (min 1)
  tNoDelay   = 2'd2   // Load: 1 (minimum delay)
} tcount_sel_e;
```
...
| ----------------------- | ------------------------- | -------------------------------------------------------------- |
| `bus_error_o`           | Tied to `'0` (TODO)       | Keep as `'0`; errors detected at flow_active level             |
| `sel_od_pp` handling    | Pass-through only         | Same (pass-through)                                            |
| Instance name           | `u_bus_tx`                | `xbus_tx`                                                      |
...
8. **OD/PP pass-through:** Verify sel_od_pp_o tracks sel_od_pp_i
9. **Idle state:** Verify SDA is HIGH and bus_tx_idle_o is asserted when no request
...
src/verification/uvm_i3c/
  sequences/
    i3c_private_write_vseq.sv   # TX byte path (address + write data)
    i3c_entdaa_vseq.sv          # TX byte path (broadcast header, assigned address)
    i3c_i2c_write_vseq.sv       # TX byte path in OD mode
  tests/
...
**Module coverage note:** `bus_tx_flow` is exercised by `i3c_private_rw_test` (address + write data), `i3c_entdaa_test` (broadcast header + CCC code + dynamic address), and `i3c_i2c_test` (I2C address + data bytes in OD mode).
...
- `bus_tx` is instantiated as `xbus_tx` in `bus_tx_flow` (not `u_bus_tx`).
